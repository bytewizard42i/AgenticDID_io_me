...
> **⚠️ Redacted for migration into the public `AgenticDID_io_me` repo.**
> All real API keys, tokens, and secrets that originally lived in this file
> have been replaced with `<REDACTED>` placeholders. The original keys were
> rotated. Pull current values from the password manager / `.env` files.
...
```
OPENAI_API_KEY=<REDACTED - rotated; pull from password manager>
OPENAI_MODEL_NAME=gpt-3.5-turbo
ANTHROPIC_API_KEY=<REDACTED - rotated; pull from password manager>
```
✅ **Status**: Ready for development

### 2. Cardano Blockchain (READY TO USE)
```
BLOCKFROST_API_KEY_PREPROD=<REDACTED - rotated; pull from password manager>
NETWORK=Preprod
