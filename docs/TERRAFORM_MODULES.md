...
Provisions, for AgenticDID: a per-app CockroachDB database, a least-privilege
login role, table grants, and (optionally) the connection string in AWS Secrets
Manager. As the agent-authority branch, AgenticDID owns agent-grant writes.
...
module "helixchain" {
  ...
  app_name     = "agenticdid"
  db_password  = var.agenticdid_db_password   # TF_VAR_agenticdid_db_password
  cluster_host = var.cluster_host
  grants = {
    agent_grants = ["SELECT", "INSERT", "UPDATE"]   # grant/delegate/revoke
    identities   = ["SELECT"]                       # resolve principals
  }
}
```
